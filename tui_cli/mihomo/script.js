...
const DIRECT_PROCESS_NAMES = ["steam.exe"];
...
// 强制走代理的精确域名。
const FORCE_PROXY_DOMAINS = ["v2rayse.com"];
...
const NSFW_DOMAIN_SUFFIXES = ["88newline.jb-aiwei.cc"];
...
const buildProxyGroups = ({ allNames, aiNames, infoNames }) => {
	...
	if (allNames.length) {
		...
		add(
			"全部",
			"select",
			["BetterFB - 全部", "URL Test - 全部", ...allNames],
			"Available.png",
		);
	}
...
const applyDns = (cfg) => {
	const dns = cfg.dns || {};
	const fakeIpFilter = Array.isArray(dns["fake-ip-filter"])
		? dns["fake-ip-filter"]
		: [];
